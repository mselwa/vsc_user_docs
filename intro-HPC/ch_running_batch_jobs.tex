...
Job ID      Name    User      Time Use S Queue
----------- ------- --------- -------- - -----
%\jobnumber%.%\dots% mpi     %\userid% 0        Q q1h
\end{prompt}
\fi
...
\ifleuven
\begin{prompt}
%\shellcmd{qsub -l nodes=1:haswell}%
\end{prompt}

The job requires just one node, but it should have an Intel Haswell processor.
A list with site-specific properties can be found in the next section.
\fi
...
\ifleuven
\begin{prompt}
%\shellcmd{qsub -l nodes=1:haswell,mem=2gb fibo.pbs}%
\end{prompt}
\fi
...
\begin{tabular}{|p{0.7in}|p{5.3in}|} \hline
\strong{Property} & \strong{Explanation}                                        \\ \hline
ivybridge         & only use Intel Ivy Bridge processors (E5-2680v2) \\ \hline
haswell           & only use Intel Haswell processors (E5-2680v3) \\ \hline
M2070             & only use nodes with NVIDIA Tesla M2070 GPGPUs \\ \hline
K20Xm             & only use nodes with NVIDIA Tesla K20x GPGPUs \\ \hline
K40c              & only use nodes with NVIDIA Tesla K40 GPGPUs \\ \hline
\end{tabular}

Some specific nodes can be accessed through corresponding partition request, e.g. 

\begin{prompt}
%\shellcmd{qsub -l partition=gpu}%
\end{prompt}

allows to submit the job to the GPU nodes. Similarily, 

\begin{prompt}
%\shellcmd{qsub -l partition=phi}%
\end{prompt}

requests nodes with Intel Xeon Phi 5110P co-processors and 

\begin{prompt}
%\shellcmd{qsub -l partition=visualization}%
\end{prompt}

uses visualization nodes with NVIDIA Quadro K5200 GPUs.
\fi
\ifbrussel
\begin{tabular}{|p{0.7in}|p{5.3in}|} \hline
\strong{Property} & \strong{Explanation}                                        \\ \hline
...
\ifleuven
If you do not want to specify the e-mail address each time, 
you can create the ``.forward`` file in your home directory
containing your e-mail address.
The system will use that e-mail address to send you messages.
\fi
\ifgent
